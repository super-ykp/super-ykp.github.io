...
func setDeteil() {
	this.PDetail = []Pstruct{
		...
		{5, skill.S_COIN_TO_SP, skill.S_COIN_TO_SP, "スキル\nコインをFORCEに変えるよ"},
...
func SelectPowerUp(selType int) {
	...
	case 3: //FORCE
		/*if this.Gparam.FirstPowerUp { //最初の一回は固定
			this.PLIST[0] = getPindex(skill.S_EXP_TO_GOLD)
			this.PLIST[1] = getPindex(skill.S_COIN_TO_SP)
			this.PLIST[2] = getPindex(skill.S_EXPx2)
			this.Gparam.FirstPowerUp = false
			return
		}*/
...
func PowerUp(pindex int) {
	...
	//スキルの場合、スキルをセット。再セットの場合アップグレードする
	if ptype < 100 {
