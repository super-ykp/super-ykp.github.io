...
func Update() bool {
	...
	for i := 0; i < len(it.ItemList); i++ {
		ic := &it.ItemList[i]
		//要素未使用か、回収まちでなければ、次
		if !ic.IsUse || ic.MoveMode != item.M_KAISYUMACHI {
			continue
		}
		//回収。アイテム種類による
		switch ic.ItemType {
...
func KaisyuEnemy() {
	...
	for i := 0; i < len(eL); i++ {
		...
		//敵回収待ちなら
		if ec.EStete == enemy.S_KAISYUMACHI1 || ec.EStete == enemy.S_KAISYUMACHI2 {
			//その敵は消滅
			ec.IsUse = false
...
func MagicDamage(damage int, magicID int) {
	...
	for i := 0; i < len(eL); i++ {
		...
		switch ec.EStete {
		case enemy.S_LEVING1, enemy.S_LEVING2, enemy.S_KAISYUMACHI1, enemy.S_KAISYUMACHI2:
			continue
		}
		//空中敵のみ
		...
		if ec.HP <= 0 { //体力がなくなったらダメージモーション後退場
			ec.HP = 0
			enemy.SetState(ec, enemy.S_DMGLEVING2)
		} else {
			enemy.SetState(ec, enemy.S_DMGBROW)
		}
	}
