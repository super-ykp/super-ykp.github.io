...
type PlayerStruct struct {
	...
	PState   PstateStruct //プレイヤーのステータス
	SelSkill int

	G_counter uint //グローバルカウンタ
	flg1      bool //汎用フラグ
}
...
func Init(t *PlayerStruct, p *ebiten.Image) {
	...
	//アニメ種類毎にアニメパターンを設定する。ここには攻撃判定や音の発生などの指示が混じっている
...
func drawPowerUp(screen *ebiten.Image, index int) {
	...
	} else if this.PState.Skill[index].CoolTime > 0 { //ダウンタイム中。黒
		c = color.RGBA{0, 0, 0, 255}
	} else if this.SelSkill == index { //OK!白
