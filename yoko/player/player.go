...
func SkillSet(skillID int) {
	...
	for i := 0; i < len(this.PState.Skill); i++ {
		...
		if this.PState.Skill[i].SkillID == -1 {
			...
			else if this.PState.Skill[i].SkillID == skillID { //既存箇所に配置。アップグレード
				this.PState.Skill[i].Level++
				this.PState.Skill[i].CoolTimeMax -= 1//仮。ここには来ない
				if this.PState.Skill[i].CoolTimeMax < sDetail.CoolMin {
					this.PState.Skill[i].CoolTimeMax = sDetail.CoolMin
				}
				if this.PState.Skill[i].CoolTimeMax < this.PState.Skill[i].CoolTime {
